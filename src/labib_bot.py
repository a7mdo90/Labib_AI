...
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, filters, ContextTypes
        ...
        # Configure logging
log_path = Path(__file__).parent.parent / "logs" / "labib_bot.log"
logging.basicConfig(
    level=logging.INFO,
...
class LabibBot:
    ...
    def setup_environment(self):
        ...
        vision_key_path = Path(__file__).parent.parent / "config" / "vision-ocr-key.json"
        if not vision_key_path.exists():
            raise FileNotFoundError(f"Google Cloud Vision API key file not found: {vision_key_path}")
    ...
    def process_pdf_page(self, pdf_path: Path, page_number: int) -> Optional[str]:
        ...
        try:
            # Set poppler path
            poppler_path = str(Path(__file__).parent.parent / "data" / "poppler-24.08.0" / "Library" / "bin")
    ...
    def process_all_textbooks(self, base_path: str = None) -> bool:
        """Process all textbook PDFs."""
        logger.info("Starting textbook processing...")
        
        if base_path is None:
            base_path = Path(__file__).parent.parent / "data" / "Textbook_pages"
        else:
